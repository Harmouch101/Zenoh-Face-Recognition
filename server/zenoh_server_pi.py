...
class Recognizer():
	def __init__(self,file_path,cascade_path):
		...
		# Setting Raspberry pi GPIO pins
		self._servoPIN = 17
		self._led = 18
		self._start = 0
		self._stop = 0
		self._authorization = 1
		GPIO.setmode (GPIO.BCM)
		GPIO.setup (self._servoPIN, GPIO.OUT)
		GPIO.setup (self._led, GPIO.OUT)
		#Turn off the led at start
		GPIO.output (self._led, GPIO.LOW)
		self._p = GPIO.PWM (self._servoPIN, 50) # GPIO 17 For 50 Hz PWM
		# Setting the servo to its original position "The door is closed"
		self._p.start (2.5)
		time.sleep (0.5)
		self._p.stop ()
	def FileRead(self):
	...
	def Recognize(self,img):
		...
		for _,face in enumerate(faces):
			...
			if (conf < 100):
				...
				self._end = time.time()
				if (int(self._end - self._start)> 11):
					self._authorization = 1
				if (self._authorization == 1):
					self._start = time.time()
					self._authorization = 0
					# open-close the door in the background
					oc_thread = Thread(target = self.open_the_door)
					oc_thread.start()
			else:
	...
	def open_the_door (self):
		...
		# After 5 seconds, turn off led
                GPIO.output(self._led, GPIO.LOW)
                time.sleep (3)
		# Closing the door after 10 seconds from opening the door
                self.close_the_door()
                return 0

	def close_the_door(self):
                print("\n[*] Closing the door !!!")	    
                p = GPIO.PWM (self._servoPIN, 50) # GPIO 17 for PWM with 50Hz
                p.start(2.5)
                position = 12.5
                # Smooth rotation 90 degrees
                while position >2.5:
